...
class SpecAugment(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.masked_fill(mask.to(device=x.device).bool(), 0)
...
class CTCLossNM:
    ...
    def __call__(self, log_probs, targets, input_length, target_length):
        ...
        if self.log_prob_weights is not None:
            log_probs = torch.mul(self.log_prob_weights, log_probs)
