...
def main(args):
    ...
    bpe_encoder = jasper_model_definition['labels'].get('bpe_encoder', None)
    ...
    featurizer_config['min_duration'] = jasper_model_definition['input'].get('min_duration', 0.1)
    featurizer_config_eval['min_duration'] = jasper_model_definition['input'].get('min_duration', 0.1)
    ...
    if args.log_prob_weighting is not None:
        with open(args.log_prob_weighting, 'r') as f:
            log_stats = json.load(f)

        occurrences = {x[0]:x[1] for x in log_stats}
        occurrences[' '] = occurrences['__eow']
        occurrences['<BLANK>'] = occurrences['__eow']
        per_label = np.array([occurrences[x] for x in dataset_vocab])
        log_prob_weights = 1.0 / per_label
        log_prob_weights = torch.tensor(log_prob_weights).float().cuda()

        ctc_loss = CTCLossNM(log_prob_weights=log_prob_weights, num_classes=len(ctc_vocab))
    else:
...
def parse_args():
    ...
    parser.add_argument("--log_prob_weighting", default=None, type=str, help='Stats for weighting probs')
