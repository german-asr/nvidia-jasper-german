...
class Manifest(object):
    def __init__(self, data_dir, manifest_paths, labels, blank_index, max_duration=None, pad_to_max=False,
                 ...
                 normalize=True, speed_perturbation=False, filter_speed=1.0, bpe_encoder=None):
        self.labels_map = dict([(labels[i], i) for i in range(len(labels))])
        self.blank_index = blank_index
        self.max_duration= max_duration

        if bpe_encoder is not None:
            self.bpe_encoder = bpe.Encoder.load(bpe_encoder)
        else:
    ...
    def parse_transcript(self, transcript):
        if self.bpe_encoder is None:
            ...
            raw_tokens = self.bpe_encoder.tokenize(transcript)
            tokens = []
            ...
            while i < len(raw_tokens):
                ...
                if t in self.labels_map.keys():
                    tokens.append(t)
                elif t in ['__sow', '__eow']:
                    if 0 < i < len(raw_tokens) - 1:
                        tokens.append(' ')

                        if raw_tokens[i+1] in ['__sow', '__eow']:
                            i += 1

                i += 1

            transcript = [self.labels_map[x] for x in tokens]
